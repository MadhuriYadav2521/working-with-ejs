...
export const renderVotingPage = async (req, res) => {
    try {
        ...
        // return  res.render('votingPage',{ rCandidate ,vname : voterName.userName})
    } catch (err) {
        return res.send(err)
    }
}
// if user is voted make sure voter dont go to voting page again, set middleware
...
export const votingPage = async (req, res) => {
    try {
        ...
        if (!candidateName) return res.render('votingPage', { rCandidate, vname: voterName.userName, votedCandidate, message: "Select one candidate!" })
        console.log(candidateName);
        const candidate = await Candidates.findOneAndUpdate({ candidateName }, { $push: { votes: id } }).exec();
        console.log(req.session.user_id);
        await candidate.save()
        return res.redirect('/voteSuccess')
...
export const renderVoteSuccess = async (req, res) => {
    try {
        const id = req.session.user_id
        const votedCandidate = await Candidates.findOne({ votes: id }).exec();
